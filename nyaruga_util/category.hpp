...
concept morphism_between = requires(Hom f, Domain a)
{
   {
      f(a)
   }
   ->std::convertible_to<Codomain>;
};
             ...
             requires(Mor f, X x)
{
   ...
   ->std::convertible_to<unwrap_template_idx<0, X>>;
};
...
concept functor = requires(make_morph<X, Y> f, T<X> tx)
{
   {
      (T<X>::fmap(f))(tx)
   }
...
||
   requires(make_morph<X, Y> f, T<X> tx)
{
   {
      (tx.fmap(f))(tx)
   }
...
concept join = morphism_between<Mor, Domain, Codomain> && requires(Mor f, Domain x)
{
   ...
   ->std::convertible_to<T<apply_mu<T, Codomain>>>;
};
...
// Tf : TX -> TY
template <typename Mor, template <class> class T, typename Domain, typename Codomain>
concept Tf = morphism_between<Mor, Domain, Codomain> && requires()
{
...
concept kleisli_morphism = morphism_between<KleisliMor, Domain, Codomain> && (detail::T_rank<T, Domain> == 0) &&
                           requires(KleisliMor f, Domain x)
{
   {
      f(x)
   }
                                ...
                                requires(KleisliMor f, Domain x)
{
   ...
   ->std::convertible_to<T<apply_mu<T, decltype(f(x))>>>;
};
...
// mu_T : (μ◦T(-))(-), in Haskell : >>=
template <template <class> class T, object X, object Y>
   requires(detail::T_rank<T, X> == 0) && (detail::T_rank<T, Y> == 0) using make_mu_T = T<Y> (*)(T<X>, make_morph<X, T<Y>>);
