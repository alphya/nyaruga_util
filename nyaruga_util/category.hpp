...
concept mu = morphism<Mor> && requires(Mor f, domain_of<Mor> x)
{
   {
      f(x)
   }
   ->std::same_as<codomain_of<Mor>>;
};
...
{
   {
      T<X>::fmap(std::declval<make_morph<X, Y>>())
   }
   ->std::same_as<T<apply_mu<T, Y>>>;
};
...
template <typename Mor, template <class> class T>
concept eta = morphism<Mor> && requires(Mor f, domain_of<Mor> x)
{
   {
      f(x)
   }
   ->std::same_as<T<codomain_of<Mor>>>;
...
template <template <class> class T, object X>
using make_eta = make_morph<X, T<X>>;

// Tf : TX -> TY
template <typename Mor, template <class> class T>
concept Tf = morphism<Mor> && requires()
{
   std::same_as<domain_of<Mor>, T<apply_mu<T, domain_of<Mor>>>>;
...
concept kleisli_morphism = morphism<KleisliMor> && requires(KleisliMor f, domain_of<KleisliMor> x)
{
   {
      f(x)
   }
   ->std::same_as<T<apply_mu<T, codomain_of<KleisliMor>>>>;
};
...
template <typename Mor, template <class> class T>
concept f_star = morphism<Mor> && requires(Mor fstar)
{
   {
      fstar(std::declval<T<apply_mu<T, domain_of<Mor>>>>())
   }
   ->std::same_as<T<apply_mu<T, codomain_of<Mor>>>>;
};
...
concept mu_T = kleisli_morphism<KleisliMor, T> && requires(Mu_T mu_t, KleisliMor f, X x)
{
   ...
   ->std::same_as<codomain_of<KleisliMor>>;
};
